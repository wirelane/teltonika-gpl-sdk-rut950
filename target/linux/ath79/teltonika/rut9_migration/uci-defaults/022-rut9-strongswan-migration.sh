...
create_remote() {
	...
	config_get type "$1" ipsec_type "tunnel"
	...
	if [ "$type" = "tunnel" ]; then
		...
		uci add_list ipsec."$section".transport="$section"_c
	fi
...
create_connection() {
	...
	move_option "$section" leftprotoport leftprotoport "$new_section"
	move_option "$section" rightprotoport rightprotoport "$new_section"
...
add_gw() {
	local section="$1"
	local hub_address="$(uci_get dmvpn $SECTION hub_address)"
	local gw

	config_get gw "$section" gateway
	[ -n "$gw" ] && return 0
	uci_set ipsec "$SECTION"_dmvpn gateway "$hub_address"
}
> /etc/config/ipsec
