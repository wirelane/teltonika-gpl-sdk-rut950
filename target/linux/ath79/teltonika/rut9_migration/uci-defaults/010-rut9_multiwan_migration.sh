	...
	local ifname="$(uci get network.$1.ifname)"
        ...
        case "$ifname" in
			...
			interface=mob1s1a1
                        ;;                                                                        
                wlan0)
			wlan_enabled="$(uci_get network $1 enabled)"
			[ "$wlan_enabled" -eq 0 ] && continue
			interface=wlan0
                        ;;                                                                        
                eth1)
			interface=wan                                                           
...
find_wan() {
	...
	case "$ifname" in
			...
			uci_set mwan3 "${2%%_*}_mob1s1a1_member_balance" interface mob1s1a1
			...
			uci_add mwan3 member "${2%%_*}_mob1s2a1_member_balance"
                        uci_set mwan3 "${2%%_*}_mob1s2a1_member_balance" interface mob1s2a1
                        ...
                        ;;
                wlan0)
			uci_add mwan3 member "${2%%_*}_wlan0_member_balance"
			uci_set mwan3 "${2%%_*}_wlan0_member_balance" interface wlan0
                        uci_set mwan3 "${2%%_*}_wlan0_member_balance" weight "$weight"

			uci add_list mwan3."$policy".use_member="${2%%_*}_wlan0_member_balance"
                        ;;                                
                eth1)                                          
			uci_add mwan3 member "${2%%_*}_wan_member_balance"
			uci_set mwan3 "${2%%_*}_wan_member_balance" interface wan        
...
move_interface() {
        case "$ifname" in
			...
			;;
                wlan0)
			chng_loaddbal_memb "$section" wlan0
                        ;;                                                              
                eth1)
...
uci_set mwan3 default_rule family ipv4
