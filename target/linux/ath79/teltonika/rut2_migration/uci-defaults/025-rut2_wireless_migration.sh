...
move_iface() {
	...
	[ "$mode" = "sta" ] && uci set wireless."$1".network=wlan0

        check_maclist(){
		local mac_value="$1"

		[[ $mac_value =~ "-" ]] && uci del_list wireless."$wifi_iface".maclist="$mac_value"

        }
	config_list_foreach "$1" maclist check_maclist
}
